  ...
  return (
    <section className="bg-gradient-to-b from-white to-green-50 py-16 px-4 md:px-12 my-20 rounded-2xl">
      ...
      <h2 className="text-3xl md:text-4xl font-extrabold text-center text-green-700 mb-14">
        Trusted by Leading{" "}
        <span className="text-green-600">Pharma Brands</span>
      </h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8 place-items-center">
        {companies.map((company, idx) => (
          <div
            key={idx}
            className="bg-white rounded-xl border border-green-100 shadow-sm hover:shadow-lg transition-all duration-300 p-4 flex items-center justify-center w-40 h-24 group"
          >
            <img
              src={company.image}
              alt={company.name}
              title={company.name}
              className="object-contain h-16 w-32 transition-transform duration-300 group-hover:scale-105 group-hover:opacity-90"
            />
          </div>
        ))}
      </div>
