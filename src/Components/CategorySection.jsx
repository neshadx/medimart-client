...
const CategorySection = () => {
  ...
  if (isLoading) {
    return <p className="text-center py-6 text-gray-500">Loading categories...</p>;
  }

  return (
    <section className="my-20 px-4 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 place-items-center">
        {categories.map((category) => (
          <div
            key={category._id}
            onClick={() => navigate(`/category/${category._id}`)}
            className="w-full max-w-xs bg-white border border-gray-200 rounded-xl shadow-md hover:shadow-lg transition duration-300 cursor-pointer"
          >
            <div className="p-4 flex flex-col items-center">
              ...
              <p className="text-sm text-gray-500">{category.totalMedicines} medicines</p>
            </div>
          </div>
        ))}
      </div>
